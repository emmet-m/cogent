...
prettyType ty = case ty of
  ...
  where
    ...
    prettyA (RecPar _ i _ _) =  annotate S.keyword "rec" <+> annotate S.typeVar (pretty i)
    prettyA (UnifVar n) = annotate S.unifVar (pretty n)
    prettyA (TypeVarBang n) = annotate S.typeVar (pretty n) <> annotate S.sigil "!"
    prettyA (RecParBang _ i _ _) = annotate S.keyword "rec" <+> annotate S.typeVar (pretty i) <> annotate S.sigil "!"
